...
class ValueIterationTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numIterations):
        ...
        actions = list(reduce(lambda a, b: set(a).union(
            b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
        policy = {}
        for state in states:
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.opts = {'actionFn': self.env.getPossibleActions,
                     'epsilon': self.epsilon, 'gamma': self.discount, 'alpha': self.learningRate}
    ...
    def runAgent(self, moduleDict, numExperiences):
        ...
        states = filter(lambda state: len(
            self.grid.getPossibleActions(state)) > 0, self.grid.getStates())
        states.sort()
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if self.question2:
            ...
            if discount != 0.9 and noise != 0.2:
                self.addMessage(
                    'Must change either the discount or the noise, not both. Returned (discount, noise) = %s' % (result,))
                return self.testFail(grades)
        else:
            try:
                ...
                self.addMessage('Did not return a (discount, noise, living reward) triple; instead analysis.%s returned: %s' % (
                    self.parameterFn, result))
                return self.testFail(grades)
        ...
        # check policy
        actionMap = {'N': 'north', 'E': 'east',
                     'S': 'south', 'W': 'west', 'X': 'exit'}
        ...
        if self.pathVisits != None and self.pathVisits not in path:
            self.addMessage(
                'Policy does not visit state %s when moving without noise.' % (self.pathVisits,))
        ...
        if self.pathNotVisits != None and self.pathNotVisits in path:
            self.addMessage(
                'Policy visits state %s when moving without noise.' % (self.pathNotVisits,))
            self.addMessage('    States visited: %s' % (path,))
            self.addMessage('    Student policy:')
            self.printPolicy(policy, False)
            self.addMessage(
    ...
    def printPolicy(self, policy, policyTypeIsGrid):
        ...
        for ybar in range(self.grid.grid.height):
            ...
            if policyTypeIsGrid:
                ...
                self.addMessage("        %s" % ("    ".join(
                    [legend[policy.get((x, y), '.')] for x in range(self.grid.grid.width)]),))
